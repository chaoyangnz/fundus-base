...
public class CharQueue implements Cloneable {
	 ...
	 *                Indicates insufficient memory for: <CODE>new char[10]
	 *                </CODE>.
	 **/
	public CharQueue() {
		size = 0;
	...
	public void expandCapacity(int minimumCapacity) {
		...
		if (data.length >= minimumCapacity)
			...
			System.arraycopy(data, head, biggerArray, head, size);
			data = biggerArray;
	 ...
	 * @exception OutOfMemoryError
	 *                Indicates insufficient memory for increasing the queue's
	...
	public void trimToSize() {
		...
		if (data.length == size)
			...
			System.arraycopy(data, head, trimmedArray, head, size);
			data = trimmedArray;
