...
public final class Chars {
	...
	public static String join(String separator, char... array) {
		...
		StringBuilder builder = new StringBuilder(len + separator.length()
				* (len - 1));
	 ...
	 * {@link #compare(char, char)}), the first pair of values that follow any
	 * common prefix, or when one array is a prefix of the other, treats the
	 * shorter array as the lesser. For example, {@code [] < ['a'] < ['a', 'b']
	 * < ['b']}.
	 * 
	 * <p>
...
//			checkElementIndex(index, size());
		...
		public int lastIndexOf(Object target) {
			...
			if (target instanceof Character) {
				int i = Chars
						.lastIndexOf(array, (Character) target, start, end);
				if (i >= 0) {
					return i - start;
				}
			}
...
//			checkElementIndex(index, size());
		...
		/* @Override */public List<Character> subList(int fromIndex, int toIndex) {
			...
			return new CharArrayAsList(array, start + fromIndex, start
					+ toIndex);
		}
