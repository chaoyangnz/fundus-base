...
import static com.richdyang.fundus.base.ArgumentAssert.state;
 ...
 * into substrings, by recognizing a <i>separator</i> (a.k.a. "delimiter") which
 * can be expressed as a single character, literal string, regular expression,
 * {@code CharMatcher}, or by using a fixed substring length. This class
 * provides the complementary functionality to {@link Joiner}.
 * 
 * <p>
 ...
 * 
 * <p>
 * <b>Warning: splitter instances are always immutable</b>; a configuration
 * method such as {@code omitEmptyStrings} has no effect on the instance it is
 * invoked on! You must store and use the new splitter instance returned by the
 * method. This makes splitters thread-safe, and safe to store as {@code static
 * final} constants (as illustrated above).
 * 
 * <pre>
 *   // Bad! Do not do this!
 ...
 * </pre>
 * 
 * The separator recognized by the splitter does not have to be a single literal
 * character as in the examples above. See the methods {@link #on(String)},
 * {@link #on(Pattern)} and {@link #on(CharMatcher)} for examples of other ways
 * to specify separators.
 * 
 * <p>
 * <b>Note:</b> this class does not mimic any of the quirky behaviors of similar
 * JDK methods; for instance, it does not silently discard trailing separators,
 * as does {@link String#split(String)}, nor does it have a default behavior of
...
public final class Splitter {
	...
	public static Splitter on(final CharMatcher separatorMatcher) {
		ArgumentAssert.notNull(separatorMatcher);

		return new Splitter(new Strategy() {
			/* @Override */public SplittingIterator iterator(Splitter splitter,
	...
	public static Splitter on(final String separator) {
		ArgumentAssert.isTrue(separator.length() != 0,
			...
			/* @Override */public SplittingIterator iterator(Splitter splitter,
				...
				return new SplittingIterator(splitter, toSplit) {
					...
					public int separatorStart(int start) {
						...
						positions: for (int p = start, last = toSplit.length()
								- delimeterLength; p <= last; p++) {
	 ...
	 *            the pattern that determines whether a subsequence is a
	...
	public static Splitter on(final Pattern separatorPattern) {
		ArgumentAssert.notNull(separatorPattern);
		ArgumentAssert.isTrue(!separatorPattern.matcher("").matches(),
	 ...
	 *             if {@code separatorPattern} is a malformed expression
	 * @throws IllegalArgumentException
	 *             if {@code separatorPattern} matches the empty string
	 */
	 ...
	 * @return a splitter, with default settings, that can split into fixed
	 *         sized pieces
	 */
	public static Splitter fixedLength(final int length) {
		ArgumentAssert.isTrue(length > 0, "The length may not be less than 1");
			...
			/* @Override */public SplittingIterator iterator(
				...
				return new SplittingIterator(splitter, toSplit) {
					...
					public int separatorStart(int start) {
						...
						return (nextChunkStart < toSplit.length() ? nextChunkStart
								: -1);
					}
	...
	/**
	 * Returns a splitter that behaves equivalently to {@code this} splitter,
	 * but automatically omits empty strings from the results. For example,
	 * {@code Splitter.on(',').omitEmptyStrings().split(",a,,,b,c,,")} returns
	 * an iterable containing only {@code ["a", "b", "c"]}.
	 * 
	 * <p>
	...
	/**
	 * Returns a splitter that behaves equivalently to {@code this} splitter,
	 * but automatically removes leading and trailing
	 * {@linkplain CharMatcher#WHITESPACE whitespace} from each returned
	 * substring; equivalent to {@code trimResults(CharMatcher.WHITESPACE)}. For
	 * example, {@code Splitter.on(',').trimResults().split(" a, b  ,c  ")}
	 * returns an iterable containing {@code ["a", "b", "c"]}.
	 * 
	 * @return a splitter with the desired configuration
	 */
	...
	/**
	 * Returns a splitter that behaves equivalently to {@code this} splitter,
	 * but removes all leading or trailing characters matching the given {@code
	...
	private abstract static class SplittingIterator extends
		 ...
		 * separatorPosition} that does not contain a separator. This method is
		 * only invoked after a call to {@code separatorStart}.
		 */
	...
	private static abstract class AbstractIterator<T> implements Iterator<T> {
		...
		public final boolean hasNext() {
			ArgumentAssert.state(state != State.FAILED);
			switch (state) {
			case DONE:
				return false;
			case READY:
