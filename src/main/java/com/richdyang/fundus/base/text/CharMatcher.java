 ...
 * {@link Matcher} does for any {@link Object}. Also offers basic text
...
public abstract class CharMatcher implements Matcher<Character> {
	 ...
	 * Unicode standard, as illustrated <a href=
	 * "http://unicode.org/cldr/utility/list-unicodeset.jsp?a=%5Cp%7Bwhitespace%7D"
	 * >here</a>. This is not the same definition used by other Java APIs. (See
	 * a <a
	 * href="http://spreadsheets.google.com/pub?key=pd8dAQyHbdewRsnE5x5GzKQ"
	...
	public static final CharMatcher WHITESPACE = anyOf(
			BREAKING_WHITESPACE_CHARS + NON_BREAKING_WHITESPACE_CHARS).or(
			inRange('\u2000', '\u200a')).precomputed();

	/**
	 * Determines whether a character is a breaking whitespace (that is, a
	 * whitespace which can be interpreted as a break between words for
	 * formatting purposes). See {@link #WHITESPACE} for a discussion of that
	 * term.
	 * 
	 * @since fundus
	 */
	public static final CharMatcher BREAKING_WHITESPACE = anyOf(
			BREAKING_WHITESPACE_CHARS).or(inRange('\u2000', '\u2006')).or(
			inRange('\u2008', '\u200a')).precomputed();
	 ...
	 * "http://unicode.org/cldr/utility/list-unicodeset.jsp?a=%5Cp%7Bdigit%7D"
	 * >Unicode</a>.
	 */
	 ...
	 * {@link Character#isWhitespace(char) Java's definition}; it is usually
	 * preferable to use {@link #WHITESPACE}. (See a <a
	 * href="http://spreadsheets.google.com/pub?key=pd8dAQyHbdewRsnE5x5GzKQ"
	 * >comparison of several definitions of "whitespace"</a>.)
	 */
	public static final CharMatcher JAVA_WHITESPACE = inRange('\u0009',
			(char) 13)
			// \ doesn't work as a char literal
			.or(inRange('\u001c', '\u0020')).or(is('\u1680')).or(is('\u180e'))
			.or(inRange('\u2000', '\u2006')).or(inRange('\u2008', '\u200b'))
			.or(inRange('\u2028', '\u2029')).or(is('\u205f')).or(is('\u3000'))
			.precomputed();

	/**
	 * Determines whether a character is a digit according to
	 * {@link Character#isDigit(char) Java's definition}. If you only care to
	 ...
	 * {@link Character#isLetter(char) Java's definition}. If you only care to
	 * match letters of the Latin alphabet, you can use {@code inRange('a',
	 * 'z').or(inRange('A', 'Z'))}.
	 */
	 ...
	 * {@link Character#isLetterOrDigit(char) Java's definition}.
	 */
	 ...
	 * {@link Character#isUpperCase(char) Java's definition}.
	 */
	 ...
	 * {@link Character#isLowerCase(char) Java's definition}.
	 */
	...
	/**
	 * Determines whether a character is an ISO control character as specified
	 * by {@link Character#isISOControl(char)}.
	 */
	 ...
	 * category is any of SPACE_SEPARATOR, LINE_SEPARATOR, PARAGRAPH_SEPARATOR,
	 * CONTROL, FORMAT, SURROGATE, and PRIVATE_USE according to ICU4J.
	 */
	public static final CharMatcher INVISIBLE = inRange('\u0000', '\u0020').or(
			inRange('\u007f', '\u00a0')).or(is('\u00ad')).or(
			inRange('\u0600', '\u0603')).or(
			anyOf("\u06dd\u070f\u1680\u17b4\u17b5\u180e")).or(
			inRange('\u2000', '\u200f')).or(inRange('\u2028', '\u202f')).or(
			inRange('\u205f', '\u2064')).or(inRange('\u206a', '\u206f')).or(
			is('\u3000')).or(inRange('\ud800', '\uf8ff')).or(
			anyOf("\ufeff\ufff9\ufffa\ufffb")).precomputed();
	...
	public static final CharMatcher SINGLE_WIDTH = inRange('\u0000', '\u04f9')
			.or(is('\u05be')).or(inRange('\u05d0', '\u05ea')).or(is('\u05f3'))
			.or(is('\u05f4')).or(inRange('\u0600', '\u06ff')).or(
					inRange('\u0750', '\u077f'))
			.or(inRange('\u0e00', '\u0e7f')).or(inRange('\u1e00', '\u20af'))
			.or(inRange('\u2100', '\u213a')).or(inRange('\ufb50', '\ufdff'))
			.or(inRange('\ufe70', '\ufeff')).or(inRange('\uff61', '\uffdc'))
			.precomputed();
	...
	public static final CharMatcher ANY = new CharMatcher() {
		...
		public boolean matchesAllOf(CharSequence sequence) {
			ArgumentAssert.notNull(sequence);
			return true;
		}
		...
		public String replaceFrom(CharSequence sequence,
			...
			StringBuilder retval = new StringBuilder(sequence.length()
					* replacement.length());
		...
		@Override
		public int countOf(CharSequence sequence) {
			return sequence.length();
		}

		@Override
		public CharMatcher and(CharMatcher other) {
			return ArgumentAssert.notNull(other);
		}

		@Override
		public CharMatcher or(CharMatcher other) {
			ArgumentAssert.notNull(other);
			return this;
		}
	...
	public static final CharMatcher NONE = new CharMatcher() {
		...
		public String replaceFrom(CharSequence sequence,
				CharSequence replacement) {
			ArgumentAssert.notNull(replacement);
			return sequence.toString();
		}
	 ...
	 * range (both endpoints are inclusive). For example, to match any lowercase
	 * letter of the English alphabet, use {@code CharMatcher.inRange('a', 'z')}
	 * .
	 * 
	 * @throws IllegalArgumentException
	 *             if {@code endInclusive < startInclusive}
	 */
	public static CharMatcher inRange(final char startInclusive,
			final char endInclusive) {
		ArgumentAssert.isTrue(endInclusive >= startInclusive);
		return new CharMatcher() {
			@Override
			public boolean matches(Character c) {
	...
	/**
	 * Returns a matcher with identical behavior to the given {@link Character}
	 * -based matcher, but which operates on primitive {@code char} instances
	 * instead.
	 */
	public static CharMatcher forPredicate(
			final Matcher<? super Character> matcher) {
		ArgumentAssert.notNull(matcher);
		if (matcher instanceof CharMatcher) {
			return (CharMatcher) matcher;
		}
		return new CharMatcher() {

			@Override
			public boolean matches(Character character) {
				return matcher.matches(ArgumentAssert.notNull(character));
			}
		};
	}
	...
	public CharMatcher negate() {
		...
		return new CharMatcher() {
			...
			@Override
			public int countOf(CharSequence sequence) {
				return sequence.length() - original.countOf(sequence);
			}
	 ...
	 */
	public CharMatcher and(CharMatcher other) {
		return new And(Arrays.asList(this, ArgumentAssert.notNull(other)));
	}
	...
	private static class And extends CharMatcher {
		...
		@Override
		public CharMatcher and(CharMatcher other) {
			List<CharMatcher> newComponents = new ArrayList<CharMatcher>(
					components);
			newComponents.add(ArgumentAssert.notNull(other));
	 ...
	 * matcher or {@code other}.
	 */
	public CharMatcher or(CharMatcher other) {
		return new Or(Arrays.asList(this, ArgumentAssert.notNull(other)));
	}
	 ...
	 * unclear whether a precomputed matcher is faster, but it certainly
	 ...
	 * matcher in an eight-kilobyte bit array. In some situations this produces
	...
	/**
	 * For use by implementors; sets the bit corresponding to each character
	 * ('\0' to '{@literal \}uFFFF') that matches this matcher in the given bit
	 * array, leaving all other bits untouched.
	 * 
	 * <p>
	...
	/**
	 * A bit array with one bit per {@code char} value, used by
	 * {@link CharMatcher#precomputed}.
	 * 
	 * <p>
	 * TODO: possibly share a common BitArray class with BloomFilter and
	 * others... a simpler java.util.BitSet.
	 */
	 ...
	 * {@link #matches} for each character, until this returns {@code false} or
	 * the end is reached.
	 * 
	 * @param sequence
	 *            the character sequence to examine, possibly empty
	 ...
	 * 
	 * @param sequence
	 *            the character sequence to examine from the beginning
	...
	/**
	 * Returns the index of the first matching character in a character
	 * sequence, starting from a given position, or {@code -1} if no character
	 * matches after that position.
	 * 
	 * <p>
	 * The default implementation iterates over the sequence in forward order,
	 ...
	 * @param start
	 *            the first index to examine; must be nonnegative and no greater
	 *            than {@code sequence.length()}
	 * @return the index of the first matching character, guaranteed to be no
	 *         less than {@code start}, or {@code -1} if no character matches
	 * @throws IndexOutOfBoundsException
	 *             if start is negative or greater than {@code
	 *             sequence.length()}
	 */
	public int indexIn(CharSequence sequence, int start) {
		int length = sequence.length();
	 ...
	 * 
	 * @param sequence
	 *            the character sequence to examine from the end
	 * @return an index, or {@code -1} if no character matches
	 */
	public int lastIndexIn(CharSequence sequence) {
		for (int i = sequence.length() - 1; i >= 0; i--) {
	...
	/**
	 * Returns a string copy of the input character sequence, with each
	 * character that matches this matcher replaced by a given replacement
	 ...
	 * 
	 * <p>
	 * The default implementation uses {@link #indexIn(CharSequence)} to find
	 * the first matching character, then iterates the remainder of the sequence
	 * calling {@link #matches(char)} for each character.
	 * 
	 * @param sequence
	 *            the character sequence to replace matching characters in
	 * @param replacement
	 *            the character to append to the result string in place of each
	 ...
	 * 
	 * <p>
	 * <b>Note:</b> If the replacement is a fixed string with only one
	 * character, you are better off calling
	 * {@link #replaceFrom(CharSequence, char)} directly.
	 * 
	 * @param sequence
	 *            the character sequence to replace matching characters in
	 * @param replacement
	 *            the characters to append to the result string in place of each
	 *            matching character in {@code sequence}
	 * @return the new string
	 */
	 ...
	 * characters this matcher matches from the beginning and from the end of
	...
	/**
	 * Returns a substring of the input character sequence that omits all
	 * characters this matcher matches from the beginning of the string. For
	 * example:
	 * 
	 * <pre>
	 * @code
	...
	/**
	 * Returns a substring of the input character sequence that omits all
	 * characters this matcher matches from the end of the string. For example:
	 * 
	 * <pre>
	 * @code
	 * 
	 ...
	 * ... returns {@code "abacat"}.
	 */
	public String trimTailingFrom(CharSequence sequence) {
		int len = sequence.length();
		int last;
	...
	public String collapseFrom(CharSequence sequence, char replacement) {
		...
		StringBuilder builder = new StringBuilder(sequence.length()).append(
				sequence.subSequence(0, first)).append(replacement);
		boolean in = true;
	 ...
	 * does, except that groups of matching characters at the start or end of
