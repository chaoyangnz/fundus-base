...
public final class Doubles {
   ...
   * Double#compareTo compareTo}(b)</code>. As with that method, {@code NaN}
	 * is treated as greater than all other values, and {@code 0.0 > -0.0}.
	 * 
	 * @param a
	 ...
	 * 
	 * @param array
	 *            an array of {@code double} values, possibly empty
	 * @param target
	 ...
	 * {@code array}. Note that this always returns {@code -1} when {@code
	 * target} is {@code NaN}.
	 * 
	 * @param array
	 *            an array of {@code double} values, possibly empty
	 ...
	 */
	public static int indexOf(double[] array, double[] target) {
		ArgumentAssert.notNull(array, "array");
		ArgumentAssert.notNull(target, "target");
		if (target.length == 0) {
			return 0;
		}
	 ...
	 * 
	 * @param array
	 *            a <i>nonempty</i> array of {@code double} values
	...
	public static double min(double... array) {
		ArgumentAssert.isTrue(array.length > 0);
		double min = array[0];
	 ...
	 * 
	 * @param arrays
	 *            zero or more {@code double} arrays
	...
	public static double[] ensureCapacity(double[] array, int minLength,
			int padding) {
		ArgumentAssert.isTrue(minLength >= 0, "Invalid minLength: %s", minLength);
		ArgumentAssert.isTrue(padding >= 0, "Invalid padding: %s", padding);
	...
	private static double[] copyOf(double[] original, int length) {
		...
		System.arraycopy(original, 0, copy, 0, Math
	 ...
	 * by {@code separator}. For example, {@code join("-", 1.0, 2.0, 3.0)}
	...
	public static String join(String separator, double... array) {
		ArgumentAssert.notNull(separator);
		if (array.length == 0) {
			return "";
		}
	 ...
	 * {@link #compare(double, double)}), the first pair of values that follow
	 * any common prefix, or when one array is a prefix of the other, treats the
	 * shorter array as the lesser. For example, {@code [] < [1.0] < [1.0, 2.0]
	 * < [2.0]}.
	 * 
	 * <p>
	 ...
	 * 
	 * @param collection
	 *            a collection of {@code Double} objects
...
//			checkElementIndex(index, size());
		...
		/* @Override */public List<Double> subList(int fromIndex, int toIndex) {
			int size = size();
			ArgumentAssert.indexInBounds(fromIndex, toIndex, size);
			if (fromIndex == toIndex) {
				return Collections.emptyList();
			}
			return new DoubleArrayAsList(array, start + fromIndex, start
					+ toIndex);
		}
