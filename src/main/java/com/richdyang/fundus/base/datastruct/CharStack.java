...
public final class CharStack {
     ...
     *
     * @param capacity 堆栈容量
     * @throws IllegalArgumentException 当容量小于0时，抛出此异常
     * @see #CharStack()
     */
    public CharStack(int capacity) throws IllegalArgumentException {
        notNull(capacity, "stack capacity {0} lower than 0", capacity);
        this.size = 0;
     ...
     *
     * @return 返回栈顶字符
     * @throws EmptyStackException 当堆栈为空时，抛出此异常
     * @see #peek()
     */
     ...
     * @param index 索引
     * @return 堆栈中指定索引位置的字符
     * @throws IndexOutOfBoundsException 当index &gt;= size() || index &lt; 0时抛出此异常
     */
    ...
    private void ensureCapacity() {
        synchronized (this.buffer) {
            if (this.size >= this.buffer.length - 1) {
                ...
                arraycopy(this.buffer, 0, buffer, 0, this.buffer.length);
                this.buffer = buffer;
            }
        }
    }
